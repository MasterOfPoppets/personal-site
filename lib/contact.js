(function () {
  ...
  Contact.prototype.validateContactForm = function () {
    ...
    if (!this.payload.enquiry_name) {
      errors.push({enquiry_name: 'Name is required'});
    }
    
    if (!this.payload.enquiry_email) {
      errors.push({enquiry_email: 'Email address is required'});
    } else if (!validator.isEmail(this.payload.enquiry_email)) {
      errors.push({enquiry_email: 'Invalid email address'}); 
    }
    
    if (!this.payload.enquiry_message) {
      errors.push({enquiry_message: 'Message is required'});
    }
    ...
    return transporter.sendMail(
      {
        ...
        to: 'gurrkin@garbagetestmail.com',
